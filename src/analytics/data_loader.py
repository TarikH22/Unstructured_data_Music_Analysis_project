...
MONGO_EXPORT_DIR = ROOT_DIR / "mongo_backup_export" / "music_pipeline"
...
def load_bson_export_collection(collection_name: str) -> pd.DataFrame:
    """Load a collection from mongoexport BSON backups when MongoDB is unavailable."""
    bson_path = MONGO_EXPORT_DIR / f"{collection_name}.bson"
    if not bson_path.exists() or bson_path.stat().st_size == 0:
        return pd.DataFrame()

    payload = bson_path.read_bytes()
    records = decode_all(payload)
    if not records:
        return pd.DataFrame()
...
def load_available_mongo_collections(
    ...
    try:
        ...
        logger.warning("MongoDB unavailable, using BSON export fallback: %s", exc)
    finally:
        client.close()
    ...
    for name in preferred:
        if name in available:
            ...
            df = load_bson_export_collection(name)
